...
DB_PATH = BASE_DIR / os.getenv("SQLITE_PATH", "petgato_web.db")
...
def users_page(request: Request, edit_id: int | None = None):
    user = current_user(request)
    if not user:
        return RedirectResponse(url="/login", status_code=303)
    ...
    return templates.TemplateResponse(
        "users.html",
        {"request": request, "user": user, "current_section": "users", "users": users, "edit_user": edit_user, "profiles": profiles},
    )


@app.post("/users")
def create_user(request: Request, username: str = Form(...), full_name: str = Form(...), password: str = Form(...), profile_name: str = Form(...), active: str = Form("1")):
    user = current_user(request)
    if not user:
        return RedirectResponse(url="/login", status_code=303)
    blocked = guard_section(user, "users")
    ...
    conn.execute(
        ...
        (username.strip(), full_name.strip(), hash_password(password), profile_name.strip(), 1 if active == "1" else 0),
    )
    conn.commit()
    conn.close()
...
def edit_user(user_id: int, request: Request, username: str = Form(...), full_name: str = Form(...), password: str = Form(""), profile_name: str = Form(...), active: str = Form("1")):
...
def backup_page(request: Request):
    ...
    backup_files = sorted(backups_dir.glob("*.db"), reverse=True)
    ...
    return templates.TemplateResponse(
        "backup.html",
        {
            ...
            "backup_files": [f.name for f in backup_files], "postgres_disabled": is_postgres(),
        },
    )
...
def create_backup(request: Request):
    ...
    backup_path = backups_dir / f"backup_petgato_{timestamp}.db"
...
def download_backup(filename: str, request: Request):
    ...
    if not file_path.exists() or file_path.suffix.lower() != ".db":
        return RedirectResponse(url="/backup", status_code=303)
...
async def restore_backup(request: Request, backup_file: UploadFile = File(...)):
    ...
    uploads_dir = BASE_DIR / "backups"
    uploads_dir.mkdir(exist_ok=True)

    temp_path = uploads_dir / f"restore_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
    content = await backup_file.read()
    temp_path.write_bytes(content)

    # valida se abre como sqlite
    test_conn = sqlite3.connect(temp_path)
    test_conn.execute("SELECT name FROM sqlite_master LIMIT 1").fetchall()
    test_conn.close()

    restore_copy = uploads_dir / f"backup_before_restore_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
    if DB_PATH.exists():
