...
def generate_app_backup_bytes():
    conn = get_conn()
    try:
        payload = {"meta": {"generated_at": datetime.now().isoformat(), "app_db_mode": APP_DB_MODE, "format_version": 1}, "tables": {}}
...
def _insert_rows(conn, table_name: str, rows):
    if not rows:
        return
...
def seed_defaults(conn):
    default_profile_permissions = {
        "Administrador": "dashboard,clients,pets,appointments,agenda,financeiro,motorista,relatorios,users,profiles,company,backup",
...
def backup_page(request: Request, msg: str = ""):
    ...
    return templates.TemplateResponse(
        "backup.html",
        {"request": request, "user": user, "current_section": "backup", "backup_files": list_backup_items(), "s3_enabled": has_s3_config(), "using_postgres": is_postgres(), "msg": msg},
    )
...
def restore_backup_from_list(request: Request, filename: str = Form(...)):
    user = current_user(request)
    if not user:
        return RedirectResponse(url="/login", status_code=303)
    blocked = guard_section(user, "backup")
